...
class TestWebSocket:
    ...
    async def test_driver_can_update_trip(self, settings):
        ...
        # Get a channel layer to send messages over
        channel_layer = get_channel_layer()
        await channel_layer.group_add(
            group=trip_id,
            channel='test_channel'
    ...
    async def test_rider_cancels_trip(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
        ...
        # Set up a channel layer to send messages over
        channel_layer = get_channel_layer()
        await channel_layer.group_add(
            group=trip_id,
            channel='test_channel'
        ...
        # Receive message on the channel layer
        response = await channel_layer.receive('test_channel')
        response_data = response.get('data')
        assert response_data['id'] == trip_id
        assert response_data['rider']['username'] == rider.username
    ...
    async def test_driver_accepts_trip(self, settings):
        settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
